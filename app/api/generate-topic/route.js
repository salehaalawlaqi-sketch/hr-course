...
Every paragraph, example, and scenario must be distinct in wording, sentence structure, and content. Never repeat the same sentence or paragraph more than once, whether within one array or across different arrays (e.g. do not reuse an "examples" entry as a "scenarios" entry). Do not reuse the same sentence template with only a word or two swapped (e.g. "Imagine you are a/an ___, and you need to ___. How would you handle this?" used twice) — vary the grammatical structure and opening of every item. Do not end multiple paragraphs or scenarios with the same or near-identical closing sentence. Each item must add new information not already stated elsewhere in the response — if you cannot think of a genuinely new point, write fewer items rather than padding with a reworded duplicate.
For the "examples" array specifically, give each item a different opening style, in this order: item 1 opens with a direct factual statement; item 2 opens with a conditional/"if" clause; item 3 (if present) opens with a question. Do not start more than one example with the same word or phrase.
For the "scenarios" array specifically, give each item a different opening style, in this order: item 1 opens by naming a specific situation or problem (not "Imagine..."); item 2 (if present) opens with a direct second-person instruction (e.g. "You are reviewing..." / "أنت تراجع...") rather than "Imagine you are...". Never start two scenarios with the same verb or phrase, and never reuse "تخيل أنك" / "Imagine you are" more than once in the whole response.`;
